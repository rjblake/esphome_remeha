...
class Remeha : public PollingComponent, public UARTDevice {
 public:
  ...
  Sensor *number_of_3way_valve_cycles_sensor = new Sensor();
  ...
  Remeha(UARTComponent *parent) : PollingComponent(30000), UARTDevice(parent) {}
  ...
  void getSample() {
    ...
    if (isValidCRC(readdata, n)) {
        ...
        ESP_LOGE("custom", "CRC ERROR");
    }
    
    array_to_string(readdata, 80, str);
    ESP_LOGI("custom", "Sample Data: %s", str);    
  ...
  void getCounter() {
    ...
    if (isValidCRC(readdata, n)) {
      ...
      if (power_supply_avail_hours_sensor->get_name().empty()==0) power_supply_avail_hours_sensor->publish_state(((readdata[15]*256)+readdata[16])*2); delay(100); //delay for esphome to not disconnect api
      ...
      if (number_of_3way_valve_cycles_sensor->get_name().empty()==0) number_of_3way_valve_cycles_sensor->publish_state(((readdata[19]*256)+readdata[20])*8); delay(100); //delay for esphome to not disconnect api
    ...
    ESP_LOGI("custom", "Counter1 Data: %s", str);
